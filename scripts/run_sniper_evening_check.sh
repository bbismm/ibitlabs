#!/bin/zsh
#
# sniper-evening-check — launchd-driven runner
#
# Replaces the scheduled-tasks MCP path that depends on Claude Code app being
# open. Now runs via launchd (OS-level cron) every evening at 21:10 LOCAL,
# regardless of whether Claude Code is in the foreground.
#
# Schedule: launchd plist com.ibitlabs.sniper-evening-check fires at
# 21:10 LOCAL (matches the prior scheduled-tasks cron `3 21 * * *` + jitter).
#
# Logs: ~/ibitlabs/logs/sniper-evening-check/<YYYYMMDD-HHMMSS>.log
#
# To run manually for testing:
#   ~/ibitlabs/scripts/run_sniper_evening_check.sh
#
...
LOG_DIR="$HOME/ibitlabs/logs/sniper-evening-check"
TS="$(date -u +%Y%m%d-%H%M%S)"
LOG="$LOG_DIR/$TS.log"
SKILL_FILE="$HOME/.claude/scheduled-tasks/sniper-evening-check/SKILL.md"

mkdir -p "$LOG_DIR"

{
  echo "=== sniper-evening-check run @ $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
daily evening check of the hybrid_v5.1 sniper strategy. The skill itself
